...
class FinalTest(FunctionalTestCase):
    ...
    def test_basic_cycle(self, backup_options=[], restore_options=[]):
        ...
        for filename, time, dir in [('symbolic_link', 99999, 'dir1'),  # pylint: disable=redefined-builtin
            ...
            self.check_same('{0}/testfiles/{1}/{2}'.format(_runtest_dir, dir, filename),
                            '{0}/testfiles/restore_out'.format(_runtest_dir))
            self.verify('{0}/testfiles/{1}/{2}'.format(_runtest_dir, dir, filename),
