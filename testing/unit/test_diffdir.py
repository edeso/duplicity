...
class DDTest(UnitTestCase):
    ...
    def testsig(self):
        ...
        select = selection.Select(Path("{0}/testfiles/various_file_types".format(_runtest_dir)))
        select.set_iter()
        sigtar = diffdir.SigTarBlockIter(select)
        diffdir.write_block_iter(sigtar, "{0}/testfiles/output/sigtar".format(_runtest_dir))

        i = 0
        for tarinfo in tarfile.TarFile("{0}/testfiles/output/sigtar".format(_runtest_dir), "r"):
            i += 1
    ...
    def empty_diff_schema(self, dirname):
        ...
        sigtar_fp = open("{0}/testfiles/output/sigtar".format(_runtest_dir), "rb")
        select2 = selection.Select(Path(dirname))
        select2.set_iter()
        diffdir.write_block_iter(diffdir.DirDelta(select2, sigtar_fp),
                                 "{0}/testfiles/output/difftar".format(_runtest_dir))

        size = os.stat("{0}/testfiles/output/difftar".format(_runtest_dir)).st_size
        assert size == 0 or size == 10240, size  # 10240 is size of one record
        if size != 0:
            fin = open("{0}/testfiles/output/difftar".format(_runtest_dir), "rb")
            diff_buf = fin.read()
    ...
    def test_empty_diff(self):
        ...
        self.empty_diff_schema("{0}/testfiles/various_file_types".format(_runtest_dir))
        ...
        sigtar_fp = open("{0}/testfiles/output/sigtar".format(_runtest_dir), "rb")
        select2 = selection.Select(Path("{0}/testfiles/various_file_types".format(_runtest_dir)))
        select2.set_iter()
        diffdir.write_block_iter(diffdir.DirDelta(select2, sigtar_fp),
                                 "{0}/testfiles/output/difftar".format(_runtest_dir))
    ...
    def test_empty_diff2(self):
        ...
        self.empty_diff_schema("{0}/testfiles/special_cases/neg_mtime".format(_runtest_dir))
        self.empty_diff_schema("{0}/testfiles/special_cases/no_uname".format(_runtest_dir))

    def test_diff(self):
        """Test making a diff"""
        sel1 = selection.Select(Path("{0}/testfiles/dir1".format(_runtest_dir)))
        diffdir.write_block_iter(diffdir.SigTarBlockIter(sel1.set_iter()),
                                 "{0}/testfiles/output/dir1.sigtar".format(_runtest_dir))

        sigtar_fp = open("{0}/testfiles/output/dir1.sigtar".format(_runtest_dir), "rb")
        sel2 = selection.Select(Path("{0}/testfiles/dir2".format(_runtest_dir)))
        delta_tar = diffdir.DirDelta(sel2.set_iter(), sigtar_fp)
        diffdir.write_block_iter(delta_tar,
                                 "{0}/testfiles/output/dir1dir2.difftar".format(_runtest_dir))
        ...
        for tarinfo in tarfile.TarFile("{0}/testfiles/output/dir1dir2.difftar".format(_runtest_dir),
                                       "r"):
    ...
    def test_diff2(self):
        ...
        sel1 = selection.Select(Path("{0}/testfiles/dir2".format(_runtest_dir)))
        diffdir.write_block_iter(diffdir.SigTarBlockIter(sel1.set_iter()),
                                 "{0}/testfiles/output/dir2.sigtar".format(_runtest_dir))

        sigtar_fp = open("{0}/testfiles/output/dir2.sigtar".format(_runtest_dir), "rb")
        sel2 = selection.Select(Path("{0}/testfiles/dir3".format(_runtest_dir)))
        delta_tar = diffdir.DirDelta(sel2.set_iter(), sigtar_fp)
        diffdir.write_block_iter(delta_tar,
                                 "{0}/testfiles/output/dir2dir3.difftar".format(_runtest_dir))

        buffer = b""
        tf = tarfile.TarFile("{0}/testfiles/output/dir2dir3.difftar".format(_runtest_dir), "r")
        for tarinfo in tf:
            if tarinfo.name.startswith(r"multivol_diff/"):
                buffer += tf.extractfile(tarinfo).read()
        assert 3000000 < len(buffer) < 4000000
        fout = open("{0}/testfiles/output/largefile.delta".format(_runtest_dir), "wb")
        fout.write(buffer)
        fout.close()
        assert not os.system("rdiff patch {0}/testfiles/dir2/largefile ".format(_runtest_dir) +
                             "{0}/testfiles/output/largefile.delta ".format(_runtest_dir) +
                             "{0}/testfiles/output/largefile.patched".format(_runtest_dir))
        dir3large = open("{0}/testfiles/dir3/largefile".format(_runtest_dir), "rb").read()
        patchedlarge = open("{0}/testfiles/output/largefile.patched".format(_runtest_dir), "rb").read()
    ...
    def test_dirdelta_write_sig(self):
        ...
        """
        deltadir1 = Path("{0}/testfiles/output/dir.deltatar1".format(_runtest_dir))
        deltadir2 = Path("{0}/testfiles/output/dir.deltatar2".format(_runtest_dir))
        cur_full_sigs = Path("{0}/testfiles/output/fullsig.dir1".format(_runtest_dir))

        cur_dir = Path("{0}/testfiles/dir1".format(_runtest_dir))
        ...
        for dirname in ['dir2', 'dir3', 'dir4']:
            ...
            old_dir = cur_dir
            cur_dir = Path("{0}/testfiles/".format(_runtest_dir) + dirname)

            old_full_sigs = cur_full_sigs
            cur_full_sigs = Path("{0}/testfiles/output/fullsig.".format(_runtest_dir) + dirname)

            delta1 = Path("{0}/testfiles/output/delta1.".format(_runtest_dir) + dirname)
            delta2 = Path("{0}/testfiles/output/delta2.".format(_runtest_dir) + dirname)
            incsig = Path("{0}/testfiles/output/incsig.".format(_runtest_dir) + dirname)
            ...
            assert not os.system("cmp %s %s" % (delta1.uc_name, delta2.uc_name))
