...
def write_multivol(backup_type, tarblock_iter, man_outfp, sig_outfp, backend):
    ...
    while not at_end:
        ...
        # Create volume
        try:
            log.Debug("BACKEND: " + str(config.backend))
        except:
            pass
        ...
        log.Debug("FILENAME: " + str(tdp.name))

        # write volume
        log.Debug("***WRITING VOLUME***")
        if config.encryption:
...
def restore_get_enc_fileobj(backend, filename, volume_info):
    ...
    for n in range(1, config.num_retries + 1):
        ...
        if config.ignore_errors:
            exc = duplicity.errors.BadVolumeException("Hash mismatch for: %s" % util.fsdecode(filename))
            ...
            def __next__(self):
                try:
                    ...
                    if hasattr(self.fileobj, 'name'):
                        ...
                        name = None
                    log.FatalError(_("Failed to read %s: %s") % (util.fsdecode(fn), util.uexc(e)),
...
def log_startup_parms(verbosity=log.INFO):
    ...
    log.Log("Args: %s" % ' '.join(u_args), verbosity)
...
"""), log.ErrorCode.pythonoptimize_set)
    ...
    log.Debug(_("Acquiring lockfile %s") % config.lockpath)
...
def do_backup(action):
    ...
    if not config.restart and action == "inc" and config.full_force_time is not None and \
