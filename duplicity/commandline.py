...
def parse_cmdline_options(arglist):
    ...
    def print_ver(o, s, v, p):  # pylint: disable=unused-argument
        print("duplicity %s" % (config.version))
        sys.exit(0)
    ...
    parser.add_option("--file-to-restore", "-r", action="callback", type="file",
                      ...
                      callback=lambda o, s, v, p: setattr(p.values, "restore_dir", util.fsencode(v.strip('/'))))
    ...
    parser.add_option("--log-timestamp", action="callback",
    ...
    parser.add_option("--do-not-restore-ownership", action="store_true")
    ...
    parser.add_option("--par2-volumes", type="int", metavar=_("number"))
    ...
    parser.add_option("--s3-use-glacier", action="store_true")

    # Whether to use S3 Glacier IR Storage
    parser.add_option("--s3-use-glacier-ir", action="store_true")

    # Whether to use S3 Glacier Deep Archive Storage
    parser.add_option("--s3-use-deep-archive", action="store_true")
    ...
    parser.add_option("--s3-use-server-side-kms-encryption", action="store_true", dest="s3_use_sse_kms")
    parser.add_option("--s3-kms-key-id", action="store", dest="s3_kms_key_id")
    parser.add_option("--s3-kms-grant", action="store", dest="s3_kms_grant")

    # Options for specifying region and endpoint of s3
    parser.add_option("--s3-region-name", type="string", dest="s3_region_name", action="store")
    parser.add_option("--s3-endpoint-url", type="string", dest="s3_endpoint_url", action="store")
    ...
    parser.add_option("--azure-blob-tier", type="string", metavar=_("Hot|Cool|Archive"))
    ...
    parser.add_option("--b2-hide-files", action="store_true")
    ...
    parser.add_option("--webdav-headers", metavar=_("extra headers for Webdav, like 'Cookie,name=value'"))
    ...
    parser.add_option("--mf-purge", action="store_true")

    def set_mpsize(o, s, v, p):  # pylint: disable=unused-argument
        setattr(p.values, "mp_segment_size", v * 1024 * 1024)
        setattr(p.values, "mp_set", True)
    parser.add_option("--mp-segment-size", type="int", action="callback", metavar=_("number"),
                      callback=set_mpsize)
    # volume size
    # TRANSL: Used in usage help to represent a desired number of
    # something. Example:
    ...
    def set_volsize(o, s, v, p):  # pylint: disable=unused-argument
        setattr(p.values, "volsize", v * 1024 * 1024)
        # if mp_size was not explicity given, default it to volsize
        if not getattr(p.values, 'mp_set', False):
            setattr(p.values, "mp_segment_size", int(config.mp_factor * p.values.volsize))
    ...
    parser.add_option("--no-files-changed", action="store_true", dest="no_files_changed")

    # If set, show file changes (new, deleted, changed) in the specified backup
    #  set (0 specifies latest, 1 specifies next latest, etc.)
    parser.add_option("--show-changes-in-set", type="int", metavar=_("number"))
    ...
    # by using dest="")
    for f in [x for x in dir(options) if x and not x.startswith("_")]:
        v = getattr(options, f)
    ...
    if isinstance(config.file_prefix, str):
        config.file_prefix = bytes(config.file_prefix, 'utf-8')
    if isinstance(config.file_prefix_manifest, str):
        config.file_prefix_manifest = bytes(config.file_prefix_manifest, 'utf-8')
    if isinstance(config.file_prefix_archive, str):
        config.file_prefix_archive = bytes(config.file_prefix_archive, 'utf-8')
    if isinstance(config.file_prefix_signature, str):
        config.file_prefix_signature = bytes(config.file_prefix_signature, 'utf-8')
...
''')
    ...
    log.Info(_("Using archive dir: %s") % (config.archive_dir_path.uc_name,))
    log.Info(_("Using backup name: %s") % (config.backup_name,))
...
def ProcessCommandLine(cmdline_list):
    ...
    log.Debug(_("GPG binary is %s, version %s") %
              ((config.gpg_binary or 'gpg'), config.gpg_profile.gpg_version))
