...
class ImapBackend(duplicity.backend.Backend):
    ...
    def _list(self):
        ...
        for msg in flist:
            if (len(msg) == 1):
                continue
            headers = Parser(policy=default).parsestr(msg[1].decode("unicode-escape"))  # noqa  # pylint: disable=unsubscriptable-object
            ...
            if (not (header_from is None)):
                if (re.compile("^" + self.remote_dir + "$").match(header_from)):
                    ret.append(subj)
                    log.Info("IMAP flist: %s %s" % (subj, header_from))
        return ret
    ...
    def _delete_list(self, filename_list):
        for filename in filename_list:
            flist = self.imapf(self.conn.search, None, "(SUBJECT %s)" % filename)
            flist = flist[0].split()
